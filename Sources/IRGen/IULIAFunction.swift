...
struct IULIAFunction {
  ...
  func rendered() -> String {
    ...
    if let capabilityBinding = capabilityBinding {
      capabilityBindingDeclaration = "let \(IULIAFunction.mangledIdentifierName(capabilityBinding.name)) := caller()\n"
    } else {
    ...
    if let payableValueParameter = functionDeclaration.firstPayableValueParameter {
      payableValueDeclaration = "let \(IULIAFunction.mangledIdentifierName(payableValueParameter.identifier.name)) := callvalue()\n"
    } else {
  ...
  static func mangledIdentifierName(_ name: String) -> String {
    return "_\(name)"
  }
...
extension IULIAFunction {
  ...
  func renderAssignment(lhs: Expression, rhs: Expression) -> String {
    ...
    case .variableDeclaration(let variableDeclaration):
      return "let \(IULIAFunction.mangledIdentifierName(variableDeclaration.identifier.name)) := \(rhsCode)"
    case .identifier(let identifier) where identifier.enclosingType == nil:
      return "\(IULIAFunction.mangledIdentifierName(identifier.name)) := \(rhsCode)"
    default:
  ...
  func render(_ identifier: Identifier, asLValue: Bool = false) -> String {
    ...
    return IULIAFunction.mangledIdentifierName(identifier.name)
  }
