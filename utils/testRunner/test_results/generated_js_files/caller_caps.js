...
async function test_caller_caps_positive(t_contract) { 
   let assertResult012 = {result: true, msg:""} 
   console.log(chalk.yellow("Running test_caller_caps_positive test")) 
   ...
   unsetAddr();
   process_test_result(assertResult012, "test_caller_caps_positive")
}
async function test_caller_caps_negative(t_contract) { 
   let assertResult012 = {result: true, msg:""} 
   console.log(chalk.yellow("Running test_caller_caps_negative test")) 
   let addr1 = newAddress();;
   let addr2 = newAddress();;
   await transactional_method(t_contract, 'testFrameworkConstructor', [addr1])
   //let c = Counter(addr1);;
   ...
   await assertCallerSat(assertResult012, "getValue", [], t_contract);
   unsetAddr();
   process_test_result(assertResult012, "test_caller_caps_negative")
}
...
async function run_tests(pathToContract, nameOfContract) {
    ...
    await test_caller_caps_positive(depContract_0) 
    let depContract_1 = await deploy_contract(abi, bytecode); 
    await test_caller_caps_negative(depContract_1) 
}
