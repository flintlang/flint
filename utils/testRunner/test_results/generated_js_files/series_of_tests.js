...
async function testInc_1(t_contract) { 
   let assertResult012 = {result: true, msg:""} 
   console.log(chalk.yellow("Running testInc_1 test")) 
   
   //let c = Counter();;
   await transactional_method_void(t_contract, 'increment', [])
   let v = call_method_int(t_contract, 'getValue', []);
   assertEqual(assertResult012, 1, v);
   process_test_result(assertResult012, "testInc_1")
}
async function testInc_2(t_contract) { 
   let assertResult012 = {result: true, msg:""} 
   console.log(chalk.yellow("Running testInc_2 test")) 
   
   //let c = Counter();;
   await transactional_method_void(t_contract, 'increment', [])
   let v = call_method_int(t_contract, 'getValue', []);
   assertEqual(assertResult012, 1, v);
   process_test_result(assertResult012, "testInc_2")
}
async function testInc_by2(t_contract) { 
   let assertResult012 = {result: true, msg:""} 
   console.log(chalk.yellow("Running testInc_by2 test")) 
   ...
   assertEqual(assertResult012, 2, v);
   process_test_result(assertResult012, "testInc_by2")
}
...
async function run_tests(pathToContract, nameOfContract) {
    ...
    await testInc_1(depContract_1) 
    let depContract_2 = await deploy_contract(abi, bytecode); 
    await testInc_2(depContract_2) 
    let depContract_3 = await deploy_contract(abi, bytecode); 
    await testInc_by2(depContract_3) 
    let depContract_4 = await deploy_contract(abi, bytecode); 
    await testInc_by3(depContract_4) 
}
