...
async function test_state_success(t_contract) { 
   let assertResult012 = {result: true, msg:""} 
   console.log(chalk.yellow("Running test_state_success test")) 
   
   //let c = Counter();;
   await transactional_method_void(t_contract, 'changeS2', [])
   await assertCanCallInThisState(assertResult012, "increment", [], t_contract);
   process_test_result(assertResult012, "test_state_success")
}
async function test_state_negative(t_contract) { 
   let assertResult012 = {result: true, msg:""} 
   console.log(chalk.yellow("Running test_state_negative test")) 
   
   //let c = Counter();;
   await transactional_method_void(t_contract, 'changeS2', [])
   await assertCantCallInThisState(assertResult012, "getValue", [], t_contract);
   process_test_result(assertResult012, "test_state_negative")
}
...
async function run_tests(pathToContract, nameOfContract) {
    ...
    await test_state_success(depContract_0) 
    let depContract_1 = await deploy_contract(abi, bytecode); 
    await test_state_negative(depContract_1) 
}
